...
namespace CppJieba
{
    ...
    class KeywordExtractor: public InitOnOff
    {
        ...
        public:
            ...
            bool init(const string& dictPath, const string& hmmFilePath, const string& idfPath, const string& stopWordPath,const string& userDictPath)
            {
                _loadIdfDict(idfPath);
                _loadStopWordDict(stopWordPath);
                return _setInitFlag(_segment.init(dictPath, hmmFilePath,userDictPath));
            };
        public:
			bool cut(const string& str,vector<string>& words){
				return _segment.cut(str,words);
			}
