...
var c = new Crawler({
    ...
    "callback": function(error, result, $) {
		...
		href=util.resolveRelativeURL(href,currentUrl);
                db.addUrl(href, currentUrl);
            });
            db.index(currentUrl, result, $$, function($1) {
		...
		var content=$("div#content").text();
               // console.log("content <<<< %s >>>>",content.text());
                ...
                if (content.length > 0 && title.length>0 ) {
                    ...
                    //return $$.text();
                    console.log("content.length<=0: "+title.length+" title:"+title);
                    var body="";
                    if(content.length>0){
                        body=content;
                    }else{
...
c.queue("http://www.cosmopolitan.com.hk/love-sex/guy-spy/stay-alive-men-cheating");
/*
